...
function getOtlpLogger (opts) {
  const resourceDetectors = (process.env.OTEL_NODE_RESOURCE_DETECTORS || 'env,host,os,process').split(',')
  let resources = resourceFromAttributes({ ...opts.resourceAttributes })

  if (resourceDetectors[0] !== 'none') {
    const detectors = resourceDetectors[0] === 'all'
      ? [
          envDetector,
          hostDetector,
          osDetector,
      ...
      : resourceDetectors.map(it => detectorsMapping[it])

    resources = detectResources({ detectors }).merge(resources)
  }
